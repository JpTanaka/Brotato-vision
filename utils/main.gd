func _process(delta)->void:
    if _wave_timer.time_left as int != _last_time_checked:
        ...
        # Replace the hard-coded part with an environment variable
        var project_path = OS.get_environment("PROJECT_PATH")
        var file_path = "%s/images/rt/%s_%s" % [project_path, str(current_time.minute), str(current_time.second)]
